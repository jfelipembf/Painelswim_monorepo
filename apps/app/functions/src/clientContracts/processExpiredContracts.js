...
/**
 * ============================================================================
 * EXPIRED CONTRACTS PROCESSOR
 * ____________________________________________________________________________
 *
 * 1. processExpiredContracts: Task agendada que percorre contratos e marca
 *    como 'expired' aqueles que passaram da data final.
 *
 * ============================================================================
 */
...
module.exports = functions
    ...
    .onRun(async () => {
        ...
        const processPromises = expiredSnapshot.docs.map(async (docSnap) => {
            ...
            if (!endDate) return; // Should not happen based on query, but safety first
            ...
            // 2. Clean up future enrollments (Post-transaction to avoid complexity limits)
            // Similar logic to cancellation cleanup
            try {
                ...
                enrollmentsSnap.docs.forEach((enrollDoc) => {
                    ...
                    // If it's a single session AFTER the contract end date, delete it
                    if (enroll.type === "single-session" && enroll.sessionDate > endDate) {
                        batch.delete(enrollDoc.ref);
                        batchCount++;
                    }
                    // If it's recurring, we might want to check effectiveFrom? 
                    // Usually recurring stays until cancelled. 
                    // If the contract finished, the enrollment should likely end or be removed if it has no other valid contract.
                    // For now, let's correspond to the cancellation logic:
                    // Cancellation removes ALL enrollments. 
                    // Expiration implies the period is done. 
                    // Let's remove recurring enrollments too as they shouldn't exist without an active contract.
