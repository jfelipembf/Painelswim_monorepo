...
import { getFinancialDashboardData } from "../../modules/dashboard";
import type { FinancialDashboardData } from "../../modules/dashboard";
...
export const useFinancialDashboard = () => {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      const result = await getFinancialDashboardData(idTenant, idBranch, selectedDate);
      if (requestIdRef.current !== requestId) return;
      setData(result);
    } catch (err: any) {
      if (requestIdRef.current !== requestId) return;
      setError(err.message || "Erro ao carregar dashboard financeiro");
    } finally {
