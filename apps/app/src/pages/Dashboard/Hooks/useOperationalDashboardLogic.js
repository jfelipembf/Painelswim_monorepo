...
import { getStaffDailyStats, getStaffMonthlyStats, getStaffDailyExperimentals, getBirthdaySummary } from "../../../services/Summary/operational.service"
        ...
        try {
            await withLoading('page', async () => {
                const [dailyRes, monthlyRes, expListRes, tasksRes, bdayRes] = await Promise.allSettled([
                    ...
                    getStaffDailyExperimentals(user.uid),
                ...
                const monthly = monthlyRes.status === 'fulfilled' ? monthlyRes.value : { totalAmount: 0 }
    ...
    const reports = useMemo(() => {
        return [
            ...
            },
            {
                ...
                total: formatCurrency(stats.todaySalesAmount),
                badgecolor: "info",
                average: formatCurrency(stats.monthSalesAmount),
                label: "No Mês"
            },
            {
